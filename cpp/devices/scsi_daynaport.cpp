...
vector<uint8_t> SCSIDaynaPort::InquiryInternal() const
{
	...
	// The Daynaport driver for the Mac requests 37 bytes and expects all of them to be deliverd.
...
int SCSIDaynaPort::Read(cdb_t cdb, vector<uint8_t>& buf, uint64_t)
{
	...
	while (read_count < MAX_READ_RETRIES) {
		...
		LogTrace(fmt::format("Packet Size {0}, read count: {1}", rx_packet_size, read_count));
...
void SCSIDaynaPort::Read6()
{
	...
	// If any commands have a bogus control value they were most likely sent by a non-Daynaport driver,
    // e.g. when probing for devices
	if (GetController()->GetCmdByte(5) != 0xc0 && GetController()->GetCmdByte(5) != 0x80) {
		throw scsi_exception(sense_key::illegal_request, asc::invalid_field_in_cdb);
	}

	LogTrace(fmt::format("READ(6) command, record: ${:x}", record));
